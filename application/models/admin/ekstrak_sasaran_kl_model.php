...
class Ekstrak_sasaran_kl_model extends CI_Model
{ 
	...
	function get_datatables($params){
		$this->datatables->select('kode_ss_kl,kode_kl,deskripsi, tahun ');
		$this->datatables->from('anev_sasaran_strategis');
		if (isset($params)){
	...
	function save_ekstrak($data){
		...
		foreach ($data as $update_item) {
			...
			$sql = 'INSERT INTO anev_sasaran_strategis (tahun, kode_ss_kl, deskripsi, kode_kl )
					VALUES (?,?,?,?)
					ON DUPLICATE KEY UPDATE 
						deskripsi=VALUES(deskripsi),	
						kode_kl=VALUES(kode_kl)';
			//,$update_item['pagu']
			$query = $this->db->query($sql, array( $update_item['tahun'],$update_item['kode_sasaran_kl'],$update_item['deskripsi'],$update_item['kode_kl']));  

...
class Realisasi_program extends CI_Controller {
	...
	{	
		...
		$this->load->model('laporan/realisasi_program_model','realisasi',TRUE);
	...
	function index()
	{
		...
		$template['konten']	= $this->load->view('laporan/realisasi_program_v',$data,true); #load konten template file
	...
	function get_list_rincian($tahun,$indikator,$kode_program,$kode_kegiatan,$kdlokasi,$ajaxCall=true)
	{
		...
		$data	= $this->realisasi->get_detil_belanja($params);
		if ($ajaxCall)
	...
	function print_pdf($renstra,$tahun,$indikator,$kode_program,$kode_kegiatan,$kdlokasi)
   {
		...
		$pdf->SetTitle('Rincian Kegiatan realisasi Menurut Indikator');
		...
		$pdf->AddPage();
		//var_dump($e1);
		 $pdf->Write(0, 'Rincian Kegiatan realisasi Menurut Indikator', '', 0, 'L', true, 0, false, false, 0);
	   ...
	   $data['itempekerjaan'] = $this->get_list_rincian($tahun,$indikator,$kode_program,$kode_kegiatan,$kdlokasi,false);
		...
		$html = $this->load->view('laporan/print/pdf_realisasi',$data,true);
	//	$html = $data['ikuE2'];
	//	var_dump($html);
		...
		$pdf->SetFont('helvetica', 'B', 10);	
		$pdf->Output('RincianKegiatanrealisasi.pdf', 'I');
   }
